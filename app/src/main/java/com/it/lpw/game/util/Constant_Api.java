package com.it.lpw.game.util;


import com.it.lpw.game.Config.Config;
import com.it.lpw.game.R;

public class Constant_Api {

    public static final String BANNER_SPIN = "spin";
    public static final String LINK = "link";
    public static final int GAME_MINUTE =0 ;
    public static final String UPDATE = "update";
    public static final String MAINTENANCE = "maintenance";
    public static String SHARE_MSG ="" ;
    public static boolean INTERSTITIAL = false;
    public static int INTER_COUNT =0 ;
    public static int COUNT = 0;
    public static String TID = "";
    public static String API_TYPE = "";
    public static String ADTYPE ="" ;
    public static String INTERSTITAL_ID = "";
    public static String INTERSTITAL_TYPE ="" ;
    public static String TITLE = "";
    public static String ADCOLONY_ZONEID="";
    public static String ADCOLONY_APPID="";
    public static String UNITY_GAMEID="";
    public static String UNITY_REWARD_ID ="";
    public static String BANNER_TYPE ="";
    public static String BANNER_ID ="";
    public static boolean UNITY_REWARD=true;
    public static String STARTAPP_ID="";
    public static String APPLOVIN_REWARD_ID="";
    public static boolean APPLOVIN_REWARD=false;
    public static boolean ADCOLONY_REWARD=false;
    public static boolean STARTAPP_REWARD=false;
    public static String PRIVACY_POLICY_URL="";
    public static String ABOUT_COMPANY = "";
    public static String ABOUT_VERSION = "";
    public static String ABOUT_EMAIL = "";
    public static String ABOUT_WEBSITE = "";
    public static String ABOUT_PHONE = "";
    public static String ABOUT_INFO = "";
    public static String TELEGRAM = "";
    public static String FB = "";
    public static String BANNER_TYPE_STARTAPP = "startapp";
    public static String BANNER_TYPE_UNITY = "unity";
    public static String BANNER_TYPE_APPLOVIN = "applovin";
    public static final String API="api/v1/";
    public static final String LOAD_IMAGES ="/images/";
    public static final String AUTH = "Bearer ";
    public static final String Authorization = "Authorization";
    public static final String TOKEN = "token";
    public static final String CONTENT_TYPE = "application/json";
    public static final String SHOW = "show";
    public static final String HIDE = "hide";
    public static final String CLOSE = "close";
    public static final String RESTART = "restart";
    public static final String MYTOKEN ="Authorization" ;
    public static final String OLDTOKEN = "Basic ";
    public static final String MY_DEVICE = "WkdWMmFXTmxYMmxr";
    public static String P1 = "";
    public static String P2 = "";
    public static String P3 = "";
    public static String P4 = "";
    public static String P5 = "";
    public static String P6 = "";
    public static String P7 = "";

}
