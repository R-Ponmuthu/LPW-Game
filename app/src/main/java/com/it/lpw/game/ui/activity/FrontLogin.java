...
import com.it.lpw.game.databinding.ActivityFrontLoginBinding;
import com.it.lpw.game.databinding.DialogForgotPasswordBinding;
...
public class FrontLogin extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loading= Method.loading(activity);
    ...
    public void Forgot(View view) {
        ...
        passwordBinding.submit.setOnClickListener(view1 -> {
            Method.hideKeyboard(view,activity);
                ...
                if(response.isSuccessful()){
                    if(response.body().getResponse()==201){
                        session.saveUserData(response.body().getUser().getCustId(),
                                ...
                                Constant_Api.AUTH+Method.encrypt(response.body().getWkdWMmFXTmxYMmxr()).replace(response.body().getUser().getToken(),""));
